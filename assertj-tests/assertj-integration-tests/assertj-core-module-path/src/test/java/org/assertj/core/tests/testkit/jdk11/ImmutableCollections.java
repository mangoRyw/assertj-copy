 ...
 */
package org.assertj.core.tests.testkit.jdk11;
...
class ImmutableCollections {
  ...
  static final class Map1<K, V> extends AbstractImmutableMap<K, V> {
    ...
    public Set<Entry<K, V>> entrySet() {
      return Jdk11.Set.of(new KeyValueHolder<>(k0, v0));
    }
    ...
    class MapNIterator implements Iterator<Entry<K, V>> {
      ...
      public Entry<K, V> next() {
        if (hasNext()) {
          ...
          @SuppressWarnings("unchecked")
          Entry<K, V> e = new KeyValueHolder<>((K) table[idx], (V) table[idx + 1]);
          remaining--;
          return e;
        } else {
          throw new NoSuchElementException();
...
final class CollSer implements Serializable {
  ...
  private Object readResolve() throws ObjectStreamException {
    try {
      ...
      switch (tag & 0xff) {
      case IMM_LIST:
        return Jdk11.List.of(array);
      case IMM_SET:
        return Jdk11.Set.of(array);
      case IMM_MAP:
        if (array.length == 0) {
