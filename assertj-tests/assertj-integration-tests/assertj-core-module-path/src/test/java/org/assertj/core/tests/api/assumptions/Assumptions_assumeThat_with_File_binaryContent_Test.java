...
class Assumptions_assumeThat_with_File_binaryContent_Test {
  ...
  void should_run_test_when_assumption_using_file_binaryContent_succeeds() {
    // WHEN
    ThrowingCallable assumptionCode = () -> assumeThat(FILE).binaryContent().contains("actual".getBytes(UTF_8));
    // THEN
    thenCode(assumptionCode).doesNotThrowAnyException();
  }

  @Test
  void should_ignore_test_when_assumption_using_file_binaryContent_fails() {
    // WHEN
    ThrowingCallable assumptionCode = () -> assumeThat(FILE).binaryContent().contains("foo".getBytes(UTF_8));
    // THEN
